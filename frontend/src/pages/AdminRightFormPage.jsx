...
export default function AdminRightFormPage() {
  const { id } = useParams();
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link to="/admin/derechos" className="text-azul text-sm hover:underline mb-4 inline-block">
        &larr; Volver
      </Link>
      <h1 className="text-2xl font-extrabold mb-6">
        {isEdit ? 'Editar derecho' : 'Nuevo derecho'}
      </h1>

      {error && <p className="text-red-600 text-sm mb-4 bg-red-50 rounded-lg p-3">{error}</p>}

      <div className="card">
        <RightForm initial={initial} onSubmit={handleSubmit} loading={saving} />
      </div>
    </div>
  );
}
